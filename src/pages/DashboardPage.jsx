...
const Sidebar = () => (
  <aside className="bg-white w-64 min-h-screen p-4 flex-col justify-between hidden lg:flex border-r border-gray-100">
    ...
    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200/80">
      ...
      <Link to="/wellness-checkin" className="bg-white border border-gray-300/90 text-sm font-semibold mt-4 px-4 py-2 rounded-lg w-full hover:bg-gray-100 block text-center">
        Start now
      </Link>
    </div>
  </aside>
...
const Header = ({ theme, setTheme, user, handleLogout }) => {
  ...
  return (
    <header className="flex items-center justify-between p-6 bg-[#F0FAFA] sticky top-0 z-20 border-b border-gray-200/80">
      <div className="relative w-full max-w-xs"><div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><SearchIcon /></div><input type="text" placeholder="Search resources, topics..." className="w-full bg-white rounded-lg pl-10 pr-4 py-2 border" /></div>
      <div className="flex items-center space-x-4">
        <button onClick={() => setTheme(theme === "light" ? "dark" : "light")} className="w-9 h-9 flex items-center justify-center bg-white rounded-full border text-gray-600 hover:bg-gray-100">{theme === "light" ? <MoonIcon /> : <SunIcon />}</button>
        <button className="w-9 h-9 flex items-center justify-center bg-white rounded-full border text-gray-600 hover:bg-gray-100"><BellIcon /></button>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center p-2 hover:bg-gray-50 rounded-lg"><p>Mindful breathing guide</p><Link to="/resources" className="font-semibold text-sm text-teal-600">Read</Link></div>
                  <div className="flex justify-between items-center p-2 hover:bg-gray-50 rounded-lg"><p>Better sleep in finals week</p><Link to="/resources" className="font-semibold text-sm text-teal-600">Read</Link></div>
                </div>
              </Card>
              <MyBookings />
              <Card>
                ...
                <button onClick={() => navigate("/wellness-checkin")} className="w-full bg-purple-600 text-white font-bold py-2.5 px-4 rounded-lg hover:bg-purple-700">Start Camera</button>
              </Card>
            </div>
          </div>
        </div>
...
function MyBookings() {
  ...
  const cancelBooking = async (id, amount) => {
    ...
    alert(`We will try to refund ₹${refundable} (after 15% fee) within 24-48 hours.`);
  ...
  return (
    <Card>
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/booking" className="text-teal-600 hover:text-teal-700 font-semibold text-sm">Book new</Link>
      </div>
      {items.length === 0 ? (
          ...
          {items.map(b => (
            <div key={b.id} className="border border-gray-200 rounded-lg p-3 flex items-center justify-between">
              <div className="text-sm">
                <p className="font-semibold text-gray-800">{b.counselorName}</p>
                <p className="text-gray-600">{b.dateISO} • {b.time} • ₹{b.amount} • {b.status}</p>
              </div>
              {b.status === 'confirmed' && (
                <button onClick={() => cancelBooking(b.id, b.amount)} className="text-red-600 font-semibold text-sm hover:underline">Cancel</button>
              )}
            </div>
          ))}
        </div>
