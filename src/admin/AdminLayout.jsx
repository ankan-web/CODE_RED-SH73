/**
 * StudentCareApp.jsx
 * A refactored single-file version of your dashboard broken into smaller, testable components.
 *
 * How this file is organized (you can split each component into its own file):
 * - aiService: mock API service
 * - primitives: Icon, Card, PageHeader
 * - ChartComponent: wrapper for Chart.js (supports data function that receives canvas)
 * - Sidebar, ChatbotWidget
 * - Pages: AnalyticsPage, UsersPage, ResourcesPage, ForumPage, BookingsPage
 * - App: router and layout
 *
 * To split into separate files, create a folder `src/components/` and move components there.
 */
...
export const aiService = {
  ...
  chat: async (message) => {
    ...
    return 'This is a mock reply from the AI service.';
  },
  generateSummary: async (title) => {
    await aiService.delay(1200);
    return `AI summary for "${title}" — practical tips and core concepts.`;
  },
  translate: async (text, language) => {
    await aiService.delay(700);
    if (!language || language === 'en') return text;
    return `(${language}) ${text}`;
  },
  moderate: async (postText) => {
    await aiService.delay(500);
    const hasBad = postText.toLowerCase().includes('bad word');
    return { isFlagged: hasBad, reason: hasBad ? 'Contains inappropriate language.' : 'Content seems OK.', confidence: hasBad ? 0.95 : 0.1 };
  }
};

// ------------------ primitives & small UI components ------------------
export const Icon = ({ name, className = '' }) => {
  const icons = {
    shield: (/* svg */) => (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s-8-4.5-8-11.8A8 8 0 0 1 12 2a8 8 0 0 1 8 8.2c0 7.3-8 11.8-8 11.8z" /><path d="M12 7h.01" /></svg>
    ),
    analytics: () => (
    ...
    bookings: () => (
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 2v4" /><path d="M16 2v4" /><rect width="18" height="18" x="3" y="4" rx="2" /><path d="M3 10h18" /><path d="m14 14-2 2-2-2" /></svg>
    ),
  ...
  return <div className={className}>{Comp ? Comp() : null}</div>;
};
...
const defaultOptions = {
  ...
  scales: {
    y: { beginAtZero: true, grid: { color: '#e5e7eb', borderDash: [2, 4], drawBorder: false }, ticks: { color: '#6b7280' } },
    x: { grid: { display: false }, ticks: { color: '#6b7280' } }
  },
  interaction: { intersect: false, mode: 'index' }
};

export const ChartComponent = ({ type = 'line', data, options = {} }) => {
  const canvasRef = useRef(null);
  const chartRef = useRef(null);

  useEffect(() => {
    const ctx = canvasRef.current.getContext('2d');
    const resolvedData = typeof data === 'function' ? data(canvasRef.current) : data;

    if (chartRef.current) chartRef.current.destroy();

    chartRef.current = new Chart(ctx, { type, data: resolvedData, options: { ...defaultOptions, ...options } });
...
// ------------------ Chatbot widget ------------------
export const ChatbotWidget = ({ isVisible, onClose }) => {
  ...
  return (
    <div className="fixed bottom-24 right-8 w-80 h-96 bg-white rounded-lg shadow-2xl flex flex-col z-50 animate-fade-in-up">
      ...
      <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto text-sm space-y-3">
        {messages.map((m, i) => (<div key={i} className={`flex ${m.from === 'bot' ? 'justify-start' : 'justify-end'}`}><p className={`max-w-xs p-2 rounded-lg ${m.from === 'bot' ? 'bg-gray-200 text-gray-800' : 'bg-teal-500 text-white'}`}>{m.text}</p></div>))}
      </div>
      <div className="p-2 border-t">
        <form onSubmit={handleSend} className="flex space-x-2">
          <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} className="flex-grow text-sm p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-400" placeholder="Type your message..." />
          <button type="submit" className="flex-shrink-0 bg-teal-500 text-white p-2 rounded-md hover:bg-teal-600 flex items-center justify-center" aria-label="Send message"><Icon name="send" /></button>
        </form>
      </div>
    </div>
  );
...
export const AnalyticsPage = () => {
  const userGrowthData = { labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'], datasets: [{ data: [65, 59, 80, 81, 56, 55, 40], fill: false, borderColor: '#14b8a6', tension: 0.4, pointBackgroundColor: '#14b8a6', pointRadius: 4 }] };
  const chatbotUsageData = { labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'], datasets: [{ data: [120, 200, 150, 80, 70, 110, 130], backgroundColor: '#a7f3d0', borderColor: '#10b981', borderWidth: 1, borderRadius: 4 }] };
  ...
  const forumActivityData = (canvas) => {
    ...
    return { labels: ['Week 1', 'Week 2', 'Week 3', 'Week 4', 'Week 5', 'Week 6', 'Week 7', 'Week 8'], datasets: [{ data: [200, 250, 220, 300, 280, 350, 320, 400], backgroundColor: gradient, borderColor: '#14b8a6', borderWidth: 2, fill: true, tension: 0.4 }] };
  };
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <Card><h3 className="font-semibold text-gray-800 mb-4">User Growth</h3><div className="h-64"><ChartComponent type="line" data={userGrowthData} /></div></Card>
        <Card><h3 className="font-semibold text-gray-800 mb-4">Chatbot Usage</h3><div className="h-64"><ChartComponent type="bar" data={chatbotUsageData} /></div></Card>
      </div>

      <Card><h3 className="font-semibold text-gray-800 mb-4">Forum Activity</h3><div className="h-80"><ChartComponent type="line" data={forumActivityData} /></div></Card>
    </div>
  );
};
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 uppercase bg-gray-50"><tr><th className="px-6 py-3">User</th><th className="px-6 py-3">Email</th><th className="px-6 py-3">Status</th><th className="px-6 py-3 text-right">Actions</th></tr></thead>
            <tbody>
              {users.map(user => (
...
// ------------------ Resources (componentized rows) ------------------
const ResourceRow = ({ resource, isExpanded, onToggle, summaryData, isLoadingSummary, isLoadingTranslation, onGenerate, onTranslate }) => (
  <Fragment>
    <tr onClick={() => onToggle(resource.id)} className="bg-white border-b hover:bg-gray-50 cursor-pointer">
      <td className="px-6 py-4 font-medium text-gray-900"><div className="flex items-center justify-between"><span>{resource.title}</span><Icon name={isExpanded ? 'chevronUp' : 'chevronDown'} className="text-gray-500 ml-4" /></div></td>
      <td className="px-6 py-4 text-right"><button onClick={(e) => { e.stopPropagation(); onToggle(resource.id); }} className="font-medium text-teal-600 hover:underline">Details</button></td>
    </tr>
    {isExpanded && (
      <tr className="bg-gray-50 border-b"><td colSpan="2" className="px-6 py-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
          <div>
            <h4 className="font-semibold text-gray-700 mb-3">Associated Media</h4>
            <ul className="space-y-2">
              {resource.links.map((link, i) => (
                <li key={i} className="text-sm flex items-center gap-3"><Icon name={link.type.toLowerCase()} className="text-gray-500 flex-shrink-0" /><div><a href={link.url} target="_blank" rel="noreferrer" className="text-teal-600 hover:underline">{link.title}</a></div></li>
              ))}
            </ul>
          </div>
          <div className="border-t md:border-t-0 md:border-l border-gray-200 pl-0 md:pl-6 pt-4 md:pt-0">
            ...
            <div className="space-y-4">
              <div>
                ...
                <button onClick={() => onGenerate(resource.id, resource.title)} disabled={isLoadingSummary} className="mt-2 text-xs bg-teal-100 text-teal-700 font-semibold px-2 py-1 rounded hover:bg-teal-200 disabled:bg-gray-200 disabled:text-gray-500">{isLoadingSummary ? 'Generating...' : summaryData ? 'Regenerate' : 'Generate'}</button>
              </div>
              <div>
                <h5 className="text-xs font-semibold text-gray-500 uppercase mb-2">Translation</h5>
                <select value={summaryData?.lang || 'en'} onChange={(e) => onTranslate(resource.id, e.target.value)} disabled={!summaryData || isLoadingTranslation} className="text-xs border border-gray-300 rounded p-1 w-full disabled:bg-gray-100">
...
export const ResourcesPage = () => {
  ...
  const handleRowClick = (id) => setExpandedRow((prev) => (prev === id ? null : id));

  const handleGenerateSummary = async (resourceId, resourceTitle) => {
    setLoadingStates((p) => ({ ...p, [resourceId]: true }));
    const summary = await aiService.generateSummary(resourceTitle);
    setSummaries((p) => ({ ...p, [resourceId]: { original: summary, translated: null, lang: 'en' } }));
    setLoadingStates((p) => ({ ...p, [resourceId]: false }));
  };

  const handleTranslate = async (resourceId, language) => {
    if (!language || !summaries[resourceId]?.original) return;
    if (language === 'en') {
      setSummaries((p) => ({ ...p, [resourceId]: { ...p[resourceId], translated: null, lang: 'en' } }));
      return;
    }
    setLoadingStates((p) => ({ ...p, [`translate-${resourceId}`]: true }));
    const translated = await aiService.translate(summaries[resourceId].original, language);
    setSummaries((p) => ({ ...p, [resourceId]: { ...p[resourceId], translated, lang: language } }));
    setLoadingStates((p) => ({ ...p, [`translate-${resourceId}`]: false }));
  };
            ...
            <tbody>
              {resources.map((r) => (
                <ResourceRow key={r.id}
                  ...
                  summaryData={summaries[r.id]}
                  isLoadingSummary={!!loadingStates[r.id]}
                  isLoadingTranslation={!!loadingStates[`translate-${r.id}`]}
...
export const ForumPage = () => {
  ...
  const handleAnalyze = async (reportId) => {
    ...
    // use functional update to avoid stale closure when reading reports
  ...
  return (
    <div>
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 uppercase bg-gray-50"><tr><th className="px-6 py-3">Reported Item</th><th className="px-6 py-3">Reported By</th><th className="px-6 py-3">Status</th><th className="px-6 py-3">AI Analysis</th><th className="px-6 py-3 text-right">Actions</th></tr></thead>
            <tbody>
              {reports.map(report => (
                <tr key={report.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">{report.isAnalyzing ? <span className="text-sm text-gray-500">Analyzing...</span> : report.aiStatus ? <span className={`text-sm ${report.aiStatus.isFlagged ? 'text-red-600 font-bold' : 'text-green-600'}`}>{report.aiStatus.isFlagged ? 'Flagged' : 'Looks OK'}</span> : <span className="text-sm text-gray-400">Not analyzed</span>}</td>
                  <td className="px-6 py-4 text-right space-x-2">{report.status === 'Open' && (<button onClick={() => handleAnalyze(report.id)} disabled={report.isAnalyzing} className="font-medium text-blue-600 hover:underline disabled:text-gray-400">Analyze</button>)}<button className="font-medium text-teal-600 hover:underline">View</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
export const BookingsPage = () => {
  ...
  useEffect(() => setSelectedDay(currentDate.getDate()), [currentDate]);
  ...
  return (
    <Router>
      <div className="flex min-h-screen font-sans bg-gray-50 text-gray-800">
        ...
        <button onClick={() => setChatVisible((s) => !s)} className="fixed bottom-5 right-5 bg-teal-500 text-white p-3 rounded-full shadow-lg hover:bg-teal-600 transition-transform transform hover:scale-110 z-50" aria-label="Toggle Chatbot"><Icon name="chatbot" /></button>
      </div>
    </Router>
  );
}
